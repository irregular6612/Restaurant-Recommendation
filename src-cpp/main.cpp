...
bool signIn(AccountManager* userlist){
    std::string id,pw;
...
bool signUp(AccountManager* userlist){
...
//user main menus
bool searchPage(AccountManager* userlist){
...
bool categoriesPage(AccountManager* userlist){
    std::string category;
...
bool orderHistoryPage(AccountManager* userlist){
...
bool favoritesPage(AccountManager* userlist){
...
bool accountInfoPage(AccountManager* userlist){
...
bool modifyRestaurant(AccountManager* userlist){
...
bool hansik(AccountManager* userlist){
    //std::string input;
...
bool chinese(AccountManager* userlist){
    //std::string input;
...
int main(){
    ...
    MenuItem signInPage("Sign In",signIn);
    MenuItem signUpPage("Sign Up",signUp);
    mainPage.addMenu(&signInPage);
    mainPage.addMenu(&signUpPage);

    //Sturucture of User Main Page.
    MenuPage userMainPage("User Main Page");
    MenuItem SearchPage("searchPage",searchPage);
    MenuItem CategoryPage("CategoriesPage",categoriesPage);
    MenuItem OrderHistoryPage("OrderHistoryPage",orderHistoryPage);
    MenuItem FavoritesPage("FavoritesPage",favoritesPage);
    MenuItem AccountInfoPage("AccountInfoPage",accountInfoPage);
    MenuItem ModifyRestaurant("Modify Restaurant",modifyRestaurant);
    ...
    MenuItem HansikPage("hansikPage",hansik);
    categoriesMenuPage.addMenu(&HansikPage);
    MenuItem chinesePage("chinesePage",chinese);
...
// solid principle 점검하기
//site 의 구조에서 history를 분리해서 메멘토로 뺴자.
//l 규칙 -> interface를 사용하라.



//which is the better way to make a structure of menu.

/*
1. way -> make each class per one page and implement each function in each classes.
*/

/*
2. use a command pattern and declare a  action function in global scope
-> OOP data hiding ? How can we avoid them?
As I known, function in global scope is not recommended to us.

Also I'm 

benefit -> reduce num of classes and 

+ how can I if i want to make, 
